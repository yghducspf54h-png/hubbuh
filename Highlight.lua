...
function updateCanvasSize()
    ...
    if selectionBox then
        selectionBox.Size = UDim2.new(0, math.max(largestX - 40, 1), 0, math.max(line * lineSpace, 1))
    end
end
...
function Highlight:init(frame)
    if typeof(frame) == "Instance" and frame:IsA("Frame") then
        ...
        selectionBox = Instance.new("TextBox")
        ...
        -- Invisible native TextBox used only for mouse selection / Ctrl+C.
        -- The syntax-highlighted TextLabels remain visible underneath it.
        ...
        selectionBox.Text = ""
        selectionBox.TextColor3 = Color3.new(1, 1, 1)
        selectionBox.TextTransparency = 1
        selectionBox.BackgroundTransparency = 1
        selectionBox.BorderSizePixel = 0
        selectionBox.Font = font
        selectionBox.TextSize = textSize
        selectionBox.TextXAlignment = Enum.TextXAlignment.Left
        selectionBox.TextYAlignment = Enum.TextYAlignment.Top
        selectionBox.Position = UDim2.new(0, 40, 0, 0)
        selectionBox.Size = UDim2.new(1, -40, 1, 0)
        selectionBox.ZIndex = parentFrame.ZIndex + 2
        selectionBox.Parent = scrollingFrame

        textFrame.Parent = scrollingFrame
        lineNumbersFrame.Parent = scrollingFrame
        scrollingFrame.Parent = parentFrame

        -- Keep the selectable box synchronized with the highlighted source.
        selectionBox.Focused:Connect(function()
            selectionBox.Text = Highlight:getString()
        end)

        selectionBox.FocusLost:Connect(function()
            selectionBox.Text = ""
        end)
